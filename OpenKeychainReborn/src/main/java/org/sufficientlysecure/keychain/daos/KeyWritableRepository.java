        ...
        try {
            ...
            { // insert subkeys
                ...
                for (CanonicalizedPublicKey key : keyRing.publicKeyIterator()) {
                    ...
                    SubKey subKey = SubKey.create(masterKeyId, rank, key.getKeyId(),
                            key.getBitStrength(), key.getCurveOid(), key.getAlgorithm(), c, s, e, a,
                            key.isRevoked(), SecretKeyType.UNAVAILABLE, key.isSecure(), creation, expiry, bindingSignatureTime, key.getFingerprint());
            ...
            for (int userIdRank = 0; userIdRank < uids.size(); userIdRank++) {
                ...
                UserPacket userPacket = UserPacket.create(masterKeyId, userIdRank, type, item.userId, item.name, item.email,
                        item.comment, item.isPrimary, item.selfRevocation != null, item.attributeData);
