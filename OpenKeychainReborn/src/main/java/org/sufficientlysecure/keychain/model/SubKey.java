...
public abstract class SubKey implements KeysModel {
    ...
    public static SubKey create(long masterKeyId, long rank, long keyId, Integer keySize, String keyCurveOid,
            int algorithm, boolean canCertify, boolean canSign, boolean canEncrypt, boolean canAuth,
            boolean isRevoked, SecretKeyType hasSecret, boolean isSecure, Date creation, Date expiry,
            Date validFrom, byte[] fingerprint) {
        ...
        return new AutoValue_SubKey(masterKeyId, rank, keyId, keySize, keyCurveOid, algorithm, canCertify,
                canSign, canEncrypt, canAuth, isRevoked, hasSecret, isSecure, creationUnixTime, expiryUnixTime, validFromTime, fingerprint);
    }
